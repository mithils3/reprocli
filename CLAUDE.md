...
- Keep the paper mechanism-first: the central question is why agents cannot reproduce papers, answered by the failure-mechanism taxonomy with per-mechanism transcript evidence. The frozen eval-100 is host-probed wall-free, so run-level failures are attributed to the agent; availability appears only in dataset-construction funnel statistics. Claimed and audited counts are background data rows. Answer validity objections inline at the claim site rather than deferring them to Limitations.
