/* strip.js — the transcript's vertical burn-trace rail behaviour (drawing is
   trace.js). The rail IS the scrollbar + minimap: each round is a marker on the
   ...
   verdict button drops you on the FINAL round. Redrawn as rounds stream in. */
"use strict";
...
(function () {
  const Strip = {
    ...
    draw() {
      ...
      const live = window.RENDER.effectiveStatus(this.run) === "running";
      ...
      const svg = window.Trace.draw(this.rounds, { orientation: "v", vbW: w, vbH: h, pad: 8,
        predicted, family, live, markers: true, markerR, capR: 3.6 });
