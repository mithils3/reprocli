/* trace.js — THE SIGNATURE. One dependency-free inline-SVG primitive: a run's
   cumulative compute (H100·h) plotted against its round arc, raced against the
   indigo dashed "predicted" ceiling from the lockfile. The cyan curve recolours
   to amber the instant cumulative spend crosses that ceiling, and is capped by a
   verdict glyph. The same draw fn renders a paper-card thumbnail and the full
   vertical transcript rail (which doubles as the minimap). Also the dual-track
   took/predicted ledger bar. Colours go through style="" (var() is unreliable in
