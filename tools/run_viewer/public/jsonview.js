/* jsonview.js — turns the machine-generated JSON that caps a repro or audit run
   into a readable instrument card instead of an escaped wall of text. parse()
   cheaply rejects prose and the sink's truncated payloads (they end with a
   "…(+N chars — see agent.full.log)" note, not a closing brace). blockHtml()
   sniffs the two known shapes — the agent's self-report and the S7 auditor's
   verdict — and renders each as a card, falling back to a collapsible JSON tree
   for anything else. Also wires the one document-level click handler that toggles
   the .jnode carets the render_round.js tree emits (dead everywhere until now).
   Attaches window.JsonView; leans on RENDER.esc + RENDER.nodeHtml (loaded next). */
"use strict";
...
(function () {
  ...
  // { cls, tag, delta } — pass/fail only when both values are finite numbers
  function compare(op, m, r, tol) {
    ...
    return { cls: pass ? "pass" : "fail", tag: pass ? "WITHIN BAND" : "OUTSIDE BAND", delta };
  }
  ...
  function cheatFlags(flags) {
    if (!Array.isArray(flags) || !flags.length) return `<span class="jv-noflags">✓ no cheat flags</span>`;
    return flags.map((f) => {
      let label;
  ...
  function measurements(ms) {
    ...
    return `<table class="jv-table"><thead><tr><th>metric</th><th>observed</th><th>reference</th>` +
      `<th>scope</th><th>evidence</th></tr></thead><tbody>${rows}</tbody></table>`;
  }
  function reportCard(o) {
    ...
    return `<div class="jv-card jv-report"><div class="jv-head"><span class="badge ${badge}">${esc(o.agent_assessment)}</span>` +
      `<span class="jv-selfnote">agent's own claim — graded by the auditor, not here</span>` +
  ...
  function blockHtml(text) {
    ...
    if (typeof o.agent_assessment === "string" && Array.isArray(o.measurements))
      return { label: "🧾 final report — agent self-report", html: reportCard(o) };
