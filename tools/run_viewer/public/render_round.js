...
(function () {
  ...
  function roundCardHtml(round, arxiv, run) {
    ...
    const costChip = cost != null ? `<span class="r-cost tnum">+${cost} h${rem != null ? ` · ${rem} left` : ""}</span>` : "";
    let cap = "";
    if (isFinal && V && run) {
    ...
    return `<div class="rcard ${isFinal ? "final" : ""} ${bad ? "bad" : ""} ${spike ? "spike" : ""}" data-key="${esc(key)}" data-round="${esc(round.round_index ?? "")}">
      <div class="rcard-h"><span class="r-idx">${isFinal ? "✅ FINAL" : "round " + (round.round_index ?? "?")}</span>${exit}${spike ? `<span class="badge over" title="costliest round">⚡ spike</span>` : ""}
        ${costChip}<span class="ftime">${round.ts ? esc(R.fmtTime(round.ts)) : ""}</span></div>
      ${block("💭 reasoning", round.reasoning)}${block("🗒 assistant", round.content)}
      ${(round.calls || []).map(callHtml).join("")}${cap}</div>`;
  }
