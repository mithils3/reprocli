...
| `tests/runtime` | `test_loop_guards.py`, `test_rerun.py`, `test_run_health.py`, `test_runtime_cleanup.py`, `test_tool_loop_outputs.py` | The [tool loop](../agent-core/tool-loop.md) [guardrails](../agent-core/guardrails.md) (`runtime/loop_guards.py`, `tools/result_limits.py`), rerun selection (`runtime/rerun.py`), health/telemetry finalization (`runtime/run_health.py`), CLI arg defaults & server wiring (`vllm/server.py`, `run_arxiv_prompt_vllm.py`), and incremental output writing (`runtime/tool_loop.py`). |
