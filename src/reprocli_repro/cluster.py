...
is pinned here. The pinned account and partition both match the live ``scripts/*.sbatch``:
GPU steps and the sbatch jobs that serve the brain all charge ``betw-dtai-gh``.
"""
...
_PROFILES: dict[str, Cluster] = {
    "deltaai": Cluster(
        ...
        # carry: a sweep's brain/orchestrator node and every run_gpu hold now come
        # out of one project. NOTE the two names — the PROJECT is betw, the SLURM
        # ACCOUNT string is betw-dtai-gh (the bare project code is rejected at
        # submit with "Invalid account or account/partition combination
        # specified"), and DeltaAI's QOS string tracks the account string, so an
        # account change can park a queued job in InvalidQOS until re-issued.
