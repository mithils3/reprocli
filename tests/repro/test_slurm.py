...
class BuildAcquireTests(unittest.TestCase):
    def test_builds_a_held_salloc_for_deltaai(self):
        ...
        # Host RAM/CPU scale with the GPU count (100 GB + 18 cores per GPU);
        # --cpus-per-gpu is per-GPU so it stays 18 regardless of the count.
        self.assertIn("--mem=400G", argv)
        self.assertIn("--cpus-per-gpu=18", argv)
