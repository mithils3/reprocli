...
produces the released benchmark. Six papers with inaccessible artifacts and
fifteen that depend on paid APIs failed the audit and were replaced from the
pool.}
\label{fig:funnel}
\end{figure}
...
Artifact availability determines each paper's tier, which records how much of
the result the agent must rebuild for itself. Easy papers
release code, data, and weights, and the agent need only run or evaluate them.
Medium papers withhold the weights, and the agent must retrain before
evaluating. Hard papers withhold the code as well, and the agent must
reimplement the method from the paper alone. Compute cost is
...
the 687 verified papers, 33\% fall in Easy, 44\% in Medium, and 13\% in Hard,
while 5\% are blocked outright and 5\% are non-empirical.
...
precision. Proportional sampling would leave the Hard tier with roughly a
dozen papers, and the venue proportions reported above let readers reweight
any rate back to the venue level. Within each tier the band quotas lean
toward cheap papers because the eligible pool is bottom-heavy in compute, and
we fill every quota cheapest-first, which minimizes total benchmark cost
under the quotas and makes selection deterministic.
