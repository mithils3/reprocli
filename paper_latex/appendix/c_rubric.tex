...
\caption{Tool rounds the auditor spent per run, by verdict. Each row is scaled to its own tallest bar, and the dashed line marks the median.}
...
The distribution has a mode at 4 with 72 runs, where a run executes something
...
misses the bar. Two smaller peaks of nearly equal height sit at 0 and 8, the 53 runs an
integrity flag zeroed and the 52 runs the auditor accepted as reproductions with minor caveats.
...
\caption{Anti-cheat flags the auditor raised over the 372 graded runs, by kind
