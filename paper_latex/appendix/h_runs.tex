...
This appendix annotates ten of the 400 runs, three where the agent
...
round-81 report listed every change and the auditor raised no flags.
...
for the disclosed evaluation set, 200 SVAMP examples in the authors' bundled
file against the 300 the paper states.
...
construction with MVDF off, and the released script parses the MVDF flag with
\code{type=bool}, which reads the string \code{False} as true.
...
\caption{Run card for MiniMax-M2.7 on Neural Block Linearization (arXiv~2505.21077) at the Retrain tier, with the decisive round 99 in gold.}
...
\caption{Round 96 of MiniMax-M2.7 on Neural Block Linearization (arXiv~2505.21077), the layer-skip benchmark, and the decisive round 99 with the file the agent wrote.}
