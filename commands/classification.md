...
`REPROCLI_ENDPOINT_FILE=/path/to/vllm_endpoint.json` (the file published by
`reprocli_serve`, the serving half in `src/`); the runner resolves the URL from
any of the three, and falls back to its embedded local server when none is set.
This is the recommended path for standing up one shared server many nodes attach
to — see the [serving page](../docs/slurm/serve.md).
