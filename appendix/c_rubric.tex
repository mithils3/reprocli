...
\caption{Tool rounds the auditor spent per run, by verdict, over the 372 graded
runs. Each row is scaled to its own tallest bar, and the dashed line marks the
median.}
...
misses the bar. Two smaller peaks are close together at 0 and 8, the 53 runs an
integrity flag zeroed and the 52 runs the auditor accepted as reproductions.
...
\caption{The verdict derivation of the finalizer code. The integrity checks run
first, and the score then reads off as a band, colored as in
